/*
 * Created on 12-August-2008 at 23:12:37.
 * 
...
public class LongHashMap
{
	...
	public Object put( long key, Object value ) {
		...
		if (this.isPowerOfTwo) {
			index = (int) ((key & 0x7FFFFFFFFFFFFFFFL) & (this.buckets.length - 1));
		} else {
			index = (int) ((key & 0x7FFFFFFFFFFFFFFFL) % this.buckets.length);
		}
	...
	private void increaseSize() {
		...
		for (int i = 0; i < this.buckets.length; i++) {
			...
			while (element != null) {				
				int index;
				if (this.isPowerOfTwo) {
					index = (int) ((element.key & 0x7FFFFFFFFFFFFFFFL) & (newCapacity - 1));
				} else {
					index = (int) ((element.key & 0x7FFFFFFFFFFFFFFFL) % newCapacity);
				}
