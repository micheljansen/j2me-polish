/*
 * Created on Aug 12, 2008 at 8:13:18 PM.
 *
...
implements Externalizable
{
	 ...
	 */
	public LongList( int initialCapacity, int growthFactor ) {
		this.storedObjects = new long[ initialCapacity ];
	...
	public boolean contains( long element ) {
		for (int i = 0; i < this.size; i++) {
			long object = this.storedObjects[i];
			if ( object == element ) {
				return true;
			}
		}
	...
	public long removeElementAt( int index ) {
		...
		long removed = this.storedObjects[ index ];
	...
	public long set( int index, long element ) {
		...
		long replaced = this.storedObjects[ index ];
		this.storedObjects[ index ] = element;
		return replaced;
	}
	...
	public void trimToSize() {
		if (this.storedObjects.length != this.size ) {
			long[] newStore = new long[ this.size ];
